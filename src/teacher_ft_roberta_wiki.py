...
tokenizer = AutoTokenizer.from_pretrained('./models/teacher_ft_ea')
...
class ToxicCommentClassifier(pl.LightningModule):
    def __init__(self, n_classes: int, steps_per_epoch=None, n_epochs=None):
        super().__init__()
        self.model = RobertaForSequenceClassification.from_pretrained("./models/teacher_ft_ea", num_labels=n_classes)
...
checkpoint_dir = "./models/teacher_roberta_large_wiki_lightning"
...
pos_label = 1
positive_samples = dataset.filter(lambda x: x['Label'] == pos_label)
negative_samples = dataset.filter(lambda x: x['Label'] != pos_label)
...
train_dataset.save_to_disk('./models/teacher_roberta_large_wiki' + '/train_set')
val_dataset.save_to_disk('./models/teacher_roberta_large_wiki' + '/val_set')
test_dataset.save_to_disk('./models/teacher_roberta_large_wiki' + '/test_set')
train_dataset = load_from_disk('./models/teacher_roberta_large_wiki' + '/train_set')
val_dataset = load_from_disk('./models/teacher_roberta_large_wiki' + '/val_set')
test_dataset = load_from_disk('./models/teacher_roberta_large_wiki' + '/test_set')
...
val_loader = DataLoader(val_dataset, batch_size=16, num_workers=7, collate_fn=data_collator)
test_loader = DataLoader(test_dataset, batch_size=16, num_workers=7, collate_fn=data_collator)
...
model.model.save_pretrained('./models/teacher_roberta_large_wiki')
tokenizer.save_pretrained('./models/teacher_roberta_large_wiki')
